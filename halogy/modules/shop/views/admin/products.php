...
<table class="default clear<?php echo ($catID) ? ' order' : ''; ?>">
	<thead>
		<tr>
			<th><?php echo order_link('admin/shop/products'.(($catID) ? '/'.$catID : ''),'order','Order', (($catID) ? 5 : 4)); ?></th>
			...
			<th><?php echo order_link('admin/shop/products'.(($catID) ? '/'.$catID : ''),'deal','Deal', (($catID) ? 5 : 4)); ?></th>
			<th><?php echo order_link('admin/shop/products'.(($catID) ? '/'.$catID : ''),'featured','Featured', (($catID) ? 5 : 4)); ?></th>
	...
	<?php foreach ($products as $product): ?>
		<tr class="<?php echo (!$product['published']) ? 'draft' : ''; ?>" id="shop_products-<?php echo $product['productID']; ?>">
			<td class="col0"><?php echo $product['order']; ?></td>
			<td class="col1"><?php echo (in_array('shop_edit', $this->permission->permissions)) ? anchor('/admin/shop/edit_product/'.$product['productID'], character_limiter($product['productName'], 30)) : character_limiter($product['productName'], 30); ?></td>
			<td class="col2"><?php echo dateFmt($product['dateCreated'], 'd-M-y', '', false); ?></td>
			<td class="col3"><?php echo $product['deal'] ?></td>
			<td class="col4"><?php echo $product['featured'] ?></td>

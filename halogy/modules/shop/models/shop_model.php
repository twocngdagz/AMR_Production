...
class Shop_model extends CI_Model {
	...
	// paypal pro config
	var $APISignature = 'AqhghW7eJN3LEr7xSlKKcAj5X8woARiMktK-53yMTpT6Gv4f8VsPV2b1';
	var $APIUser      = 'robert_api1.d6interactive.com';
	var $APIPass      = '6L7YXE4FT28ZDVAK';		
...
//			if ($this->site->config['shopGateway'] == 'sagepay') $this->gateway_url = 'https://live.sagepay.com/gateway/service/vspserver-register.vsp';
	...
	function get_products($catID = '', $search = '', $featured = FALSE, $limit = '', $prod_size = '', $is_sale = '', $newly_listed = '', $productID = '')
	{
        ...
        if($is_sale == 'yes')
        {
          $this->db->where('shop_products.sale_price !=', '0.00');
        }
        ...
        if($newly_listed)
        {
          $this->db->where('shop_products.dateCreated <=', date('Y-m-d h:i:s', strtotime('+1 day')));
        }
		...
		{
			if ($value == 'price' || $value == 'productName' || $value == 'catalogueID' || $value == 'dateCreated' || $value == 'stock' || $value == 'published' || $value == 'order')
			{
				if ($key == 'orderasc')
				{
					$order = TRUE;
		...
		if (!$order)
		{
			...
			$this->db->order_by('shop_products.order', 'asc');
		}
	...
	function get_all_featured_products($limit=5)
	{
		$this->db->where('siteID', $this->siteID);
		$this->db->where('deleted', 0);
		$this->db->where('featured', 'Y');
	...
	function get_gc_discounts($code)
	{
		// default where
		$this->db->where(array('siteID' => $this->siteID, 'code' => $code, 'status' => 0));
		
		$query = $this->db->get('ha_gc_sent', 1);
	...
	function load_cart()
	{
		...
		if ($cartSession)
		{
			...
			if ($this->session->userdata('discountCode'))
			{
				$gc_discount = $this->get_gc_discounts($this->session->userdata('discountCode'));
			}
			...
			foreach ($cartSession as $key => $quantity)
			{
                ...
                if($cart[$key]['sale_price'] != '0.00')
                {
                  $productPrice = $cart[$key]['sale_price'];
                }
                else
                {
			...
			$gc_discounts = 0;
			// calculate gift certificates
			if(!empty($gc_discount))
			{
				$gc_discounts = $gc_discount['amount'];
			}
			...
			else
			{
				...
				//skipping postage from Halogy -Ritzi
				/*
				...
				*/
				...
				/* Adding Custom AMR Postage -Ritzi */
				$postage = 0; $reg = 0;
				foreach ($cartSession as $key => $quantity)
				{
					$cart[$key] = $this->unpack_item($key, $quantity);
					$user = $this->shop->get_user();
					if($user['country']==NULL) $user['country'] = 'US';
					
					$suff = ($user['country']=='US') ? '' : '_i';
					$srates = $this->core->viewall('shop_postages_amr', array('id'=>$cart[$key]['amrship']));
					if(!empty($srates['shop_postages_amr'][0]))
					{
						$srates = $srates['shop_postages_amr'][0];
					
						$first = $srates['first_cost'.$suff];
						$succ = $srates['succeeding_cost'.$suff];
						$max = $srates['max_cost'.$suff];
						...
						for($x=0; $x<$quantity; $x++)
						{
							if($postage < $max)
							{
								if($x==0 && $reg==0)
									...
									$postage = $postage + $succ;
							}
						}
						$reg = 1;
						$postage = ($postage>$max) ? $max : $postage;
					}
					/*
					$quantity = $cart[$key]['quantity'];
					$bandID = $cart[$key]['bandID'];
					...
					if($bandID==1)
					{
						...
						for($x=0; $x<$quantity; $x++)
						{
							if($tempPostage<11)
							{
								$tempPostage = $tempPostage + 1;
							}
						}
						$postage = $postage + $tempPostage;
					}
					...
					if($bandID==2)
					{
						...
						for($y=0; $y<$quantity; $y++)
						{
							if($tempPostage<12)
							{
								if($y==0 && $reg==0)
								{
									$tempPostage = $tempPostage + 5;
								}
								else
								{
									$tempPostage = $tempPostage + 3;
								}
							}
						}
						$reg = 1; 
					...
					$postage = ($postage>11) ? 11 : $postage;
					//$tempPostage = ($tempPostage>11) ? 11 : $tempPostage;
					//$postage = $tempPostage;
					*/
				}
				/* End Custom AMR Postage */
				//pr($postage); 
			...
			//calculate gc discounts on total
			if(!empty($gc_discount))
			{
				$subtotal -= $gc_discounts;
			}
			...
			$data['gc_discounts'] = $gc_discounts;
	...
	function add_variation($productID, $type, $variation = '', $varPrice = 0, $backorder=0)
	{
		$this->db->set('productID', $productID);
		$this->db->set('type', $type);
		$this->db->set('siteID', $this->siteID);
		$this->db->set('backorder', $backorder);
    ...
    {
      $this->db->where('sale_price !=', '0.00');
      $this->db->where(array('shop_products.siteID' => $this->siteID, 'deleted' => 0, 'published'=> 1, 'shop_products.productID'=>5));
	  $this->db->or_where(array('shop_products.productID'=>11));
	  $this->db->or_where(array('shop_products.productID'=>12));
	  $this->db->or_where(array('shop_products.productID'=>18));
	  $this->db->or_where(array('shop_products.productID'=>19));
	  $this->db->or_where(array('shop_products.productID'=>30));
  		//, 'shop_products.productID'=>12, 'shop_products.productID'=>18, 'shop_products.productID'=>19));
      $query = $this->db->get('shop_products');
      
      return $query->result_array();
    }
    
    function users_wishlist($userID)
    {
      $this->db->select('productID');
      $this->db->where('userID', $userID);
      
      $query = $this->db->get('shop_wishlist');
    ...
    function gift_certificate_info($id)
    {
      $this->db->where('id', $id);
      $query = $this->db->get('shop_gift_certificates');
    ...
    function get_wishlist_products($ids)
    {
      ...
      $ids = implode("','", $ids);
      $sql = "SELECT * FROM `ha_shop_products` WHERE `productID` IN ('$ids')";

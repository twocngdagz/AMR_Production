...
class Uploads {
	...
	function upload_image($thumbnail = TRUE, $maxsize = 1600, $name = 'image')
	{
		...
		if ($this->CI->upload->do_upload($name))
		{
			...
			/*
			// make browse thumbnail
			if ($thumbnail === TRUE && ($imageData['image_width'] > 220 || $imageData['image_height'] > 290))
			{
				$this->CI->image_lib->clear();
				$config['image_library'] = 'gd2';
				$config['source_image'] = $config['upload_path'].'/'.$imageData['file_name'];
				$config['create_thumb'] = true;
				$config['thumb_marker'] = "_browse";
				$config['maintain_ratio'] = true;
				$config['width'] = 220;
				$config['height'] = 290;
			...
			// make product small thumbnail
			if ($thumbnail === TRUE && ($imageData['image_width'] > 150 || $imageData['image_height'] > 200))
			{
				$this->CI->image_lib->clear();
				$config['image_library'] = 'gd2';
				$config['source_image'] = $config['upload_path'].'/'.$imageData['file_name'];
				$config['create_thumb'] = true;
				$config['thumb_marker'] = "_smallprod";
				$config['maintain_ratio'] = true;
				$config['width'] = 150;
				$config['height'] = 200;
			...
			// make product big thumbnail
			if ($thumbnail === TRUE && ($imageData['image_width'] > 450 || $imageData['image_height'] > 600))
			{
				$this->CI->image_lib->clear();
				$config['image_library'] = 'gd2';
				$config['source_image'] = $config['upload_path'].'/'.$imageData['file_name'];
				$config['create_thumb'] = true;
				$config['thumb_marker'] = "_bigprod";
				$config['maintain_ratio'] = true;
				$config['width'] = 450;
				$config['height'] = 600;
	...
	{	
		...
		if ($product)
		{
			...
			if ($query->num_rows())
			{
                ...
                $row['image_1_src'] = $row['image_1'];
                $row['image_2_src'] = $row['image_2'];
                $row['image_3_src'] = $row['image_3'];
                $row['image_4_src'] = $row['image_4'];
                $row['image_5_src'] = $row['image_5'];
